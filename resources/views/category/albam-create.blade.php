...
@section('content')
    <div class="row">
        <div class="col-md-10 m-auto">
            @role('root|admin')
            <form action="{{ route('category.tree.update', $category->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @endrole
                                ...
                                @foreach ($albams as $albam)
                                    <tr>
                                        <td class="text-center">
                                            ...
                                            <input type="checkbox" name="albams[]" {{ in_array($albam->id,$selectAlbams ) ? 'checked' : '' }} value="{{ $albam->id }}">
                                        </td>
                                        <td>
                                            {{ $albam->name }}
                                        </td>
                                        <td>
                                            <img src="{{ $albam->thumbnail }}" alt="thumbnail" width="50" height="50">
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
